...
def train(args):
    ...
    optimizerG = optim.Adam(chain(encoder.parameters(), generator.parameters()), lr=args.e_lr, betas=(0, 0.99))
    optimizerT = optim.Adam(chain(a2b_transfer.parameters(), b2a_transfer.parameters()), lr=args.t_lr, betas=(0, 0.99))
    optimizerD = optim.Adam(discriminator.parameters(), lr=args.d_lr, betas=(0, 0.99))
    ...
    for epoch in range(args.pretrained_epoch+1, args.epochs+1):
        for _iter, data in enumerate(a_train_dataLoader, 0):
            ...
            if _iter % args.log_pre_iter == 0:
                print("[Epoch/Iter] ", "{:3}/{:3} | {:3}/{:3}".format(epoch, args.epochs, _iter, len(a_train_dataLoader)),
